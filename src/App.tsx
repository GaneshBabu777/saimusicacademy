...
const routes: Route[] = ["home", "about", "features", "pricing", "faq", "contact", "login", "dashboard", "billing", "support", "terms", "privacy", "awards", "gallery"];
...
function ContactPage({ createEnquiry }: { createEnquiry: (input: EnquiryInput) => Promise<Enquiry> }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (form.message.trim().length < 12) err.message = "Message must be at least 12 characters.";
    setErrors(err); if (Object.keys(err).length) return;
    setLoading(true);
    const eq = await createEnquiry({ ...form, source: "contact" });
    setLoading(false);
    setConfirmation(`Enquiry ${eq.id} submitted. Admin notification and user auto-reply are queued.`);
  ...
  return (
    <PageShell kicker="Contact" title="Send an enquiry with validation, storage, and email queue states.">
      <form className="form-grid" onSubmit={submit} noValidate>
        ...
        <Field label="Subject" error={errors.subject}><input value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} /></Field>
        <Field label="Message" error={errors.message} wide><textarea rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} /></Field>
        <div className="sm:col-span-2">
          <button className="nav-cta" type="submit" disabled={loading}>{loading ? "Sending..." : "Send enquiry"}</button>
...
function AuthPage({ users, setUsers, setUser, pushToast }: { users: User[]; setUsers: (fn: (u: User[]) => User[]) => void; setUser: (u: User) => void; pushToast: (k: ToastKind, m: string) => void }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (mode === "forgot") { setLoading(false); pushToast("success", "Password reset email queued for delivery."); return; }
...
function DashboardPage({ user, subscription, enquiries, tickets, users, setUser, updateUser, signOut }: { user: User | null; subscription: Subscription | null; enquiries: Enquiry[]; tickets: Ticket[]; users: User[]; setUser: (u: User) => void; updateUser: (u: User) => void; signOut: () => void }) {
  const [profile, setProfile] = useState({ name: user?.name ?? "", email: user?.email ?? "", phone: user?.phone ?? "" });
  const activePlan = subscription ? getPlan(subscription.planId) : null;
  const userTickets = user ? tickets.filter((t) => t.email.toLowerCase() === user.email.toLowerCase()) : [];
  const userEnquiries = user ? enquiries.filter((eq) => eq.email.toLowerCase() === user.email.toLowerCase()) : [];
  ...
  const saveProfile = (e: FormEvent) => { e.preventDefault(); const updated = { ...user, name: profile.name, email: profile.email, phone: profile.phone }; setUser(updated); updateUser(updated); };
  return (
    <PageShell kicker="Dashboard" title={`Welcome back, ${user.name}.`}>
      <div className="dashboard-grid">
        <div className="dash-card"><h3>Subscription</h3><p>{activePlan ? `${activePlan.name} is ${subscription?.status}. Next date: ${subscription?.nextRenewal.slice(0, 10)}.` : "No active subscription yet."}</p><a className="text-link" href={routeHref("billing")}>Manage billing</a></div>
        <div className="dash-card"><h3>Support</h3><p>{userTickets.length} ticket records connected to your email.</p><a className="text-link" href={routeHref("support")}>Open support</a></div>
        <div className="dash-card"><h3>Enquiries</h3><p>{userEnquiries.length} enquiry records with queued notifications.</p><a className="text-link" href={routeHref("contact")}>Send enquiry</a></div>
      </div>
      <div className="mt-10 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <form className="dash-card" onSubmit={saveProfile}><h3>Profile management</h3><div className="mt-5 grid gap-4"><Field label="Name"><input value={profile.name} onChange={(e) => setProfile({ ...profile, name: e.target.value })} /></Field><Field label="Email"><input value={profile.email} onChange={(e) => setProfile({ ...profile, email: e.target.value })} /></Field><Field label="Phone"><input value={profile.phone} onChange={(e) => setProfile({ ...profile, phone: e.target.value })} /></Field></div><div className="mt-5 flex flex-wrap gap-3"><button className="nav-cta" type="submit">Save profile</button><button className="cta-secondary" type="button" onClick={signOut}>Sign out</button></div></form>
        <div className="dash-card"><h3>Admin-ready operations snapshot</h3><div className="ops-grid mt-5"><span>Users<strong>{users.length}</strong></span><span>Enquiries<strong>{enquiries.length}</strong></span><span>Tickets<strong>{tickets.length}</strong></span><span>Auth role<strong>{user.role}</strong></span></div><p className="mt-5 text-sm leading-6 text-cream-50">This data model can be connected to your database, payment webhooks, admin console, and email provider.</p></div>
      </div>
    </PageShell>
  );
}
...
function CheckoutModal({ request, currentUser, onClose, onComplete, pushToast }: { request: CheckoutRequest | null; currentUser: User | null; onClose: () => void; onComplete: (sub: Subscription, user: User) => void; pushToast: (k: ToastKind, m: string) => void }) {
  ...
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
  useEffect(() => { setForm((c) => ({ ...c, name: currentUser?.name ?? c.name, email: currentUser?.email ?? c.email, phone: currentUser?.phone ?? c.phone })); }, [currentUser]);
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (method === "card" && (form.card.replace(/\s/g, "").length < 12 || form.expiry.length < 4 || form.cvc.length < 3)) { setError("Enter valid card number, expiry, and CVC."); return; }
    ...
    setLoading(false); pushToast("success", `${plan.name} checkout complete. Trial is active.`); onComplete(sub, user);
  };
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <form className="checkout-modal" onSubmit={submit}>
        <div className="flex items-start justify-between gap-4"><div><p className="plan-tag">Secure checkout</p><h3 className="plan-name">{plan.name}</h3><p>{formatMoney(amount)} / {request.cycle}. Trial starts today.</p></div><button className="close-btn" type="button" onClick={onClose}>✕</button></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <Field label="Name"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></Field>
          <Field label="Email"><input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></Field>
          <Field label="Phone"><input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></Field>
          <Field label="Coupon"><input placeholder="SAI20" value={coupon} onChange={(e) => setCoupon(e.target.value)} /></Field>
        </div>
        <div className="payment-methods" role="group">{(["card", "upi", "wallet"] as PaymentMethod[]).map((m) => <button key={m} className={method === m ? "active" : ""} type="button" onClick={() => setMethod(m)}>{m}</button>)}</div>
        {method === "card" ? <div className="grid gap-4 sm:grid-cols-3"><Field label="Card number"><input inputMode="numeric" value={form.card} onChange={(e) => setForm({ ...form, card: e.target.value })} /></Field><Field label="Expiry"><input placeholder="MM/YY" value={form.expiry} onChange={(e) => setForm({ ...form, expiry: e.target.value })} /></Field><Field label="CVC"><input inputMode="numeric" value={form.cvc} onChange={(e) => setForm({ ...form, cvc: e.target.value })} /></Field></div> : null}
        {method === "upi" ? <Field label="UPI ID"><input placeholder="name@bank" value={form.upi} onChange={(e) => setForm({ ...form, upi: e.target.value })} /></Field> : null}
        {method === "wallet" ? <Field label="Wallet account"><input placeholder="Phone or wallet ID" value={form.wallet} onChange={(e) => setForm({ ...form, wallet: e.target.value })} /></Field> : null}
        {discount ? <p className="success-text">Coupon applied: {discount}% off.</p> : null}
        {error ? <p className="error-text">{error}</p> : null}
        <button className="nav-cta full mt-4" type="submit" disabled={loading}>{loading ? "Processing..." : `Confirm ${formatMoney(amount)}`}</button>
        <p className="fine-print">Payment flow is API-ready for a secure gateway.</p>
      </form>
    </div>
  );
}
...
function Chatbot({ user, createTicket, createEnquiry }: { user: User | null; createTicket: (input: TicketInput) => Promise<Ticket>; createEnquiry: (input: EnquiryInput) => Promise<Enquiry> }) {
  ...
  const respond = (intent: string) => {
    if (intent === "pricing") add("bot", "Riyaz Starter begins at INR 1,499/month. Guru Pro is best for most learners. Use SAI20 for a demo discount.");
    if (intent === "faq") add("bot", "Common topics: trials, payment methods, upgrades, cancellations, and live class support.");
    if (intent === "handoff") { setMode("handoff"); add("bot", "I can create a ticket for human support. Please share your email and issue below."); }
    if (intent === "lead") { setMode("lead"); add("bot", "Share your details and admissions will follow up with lesson options."); }
  };
  const send = (e: FormEvent) => { e.preventDefault(); if (!text.trim()) return; add("user", text.trim()); const lower = text.toLowerCase(); setText(""); if (lower.includes("price") || lower.includes("plan")) respond("pricing"); else if (lower.includes("human") || lower.includes("support")) respond("handoff"); else add("bot", "I can help with pricing, FAQ, lead capture, or support tickets."); };
  return (
    <div className="chatbot">
      {open ? <div className="chat-window"><div className="chat-header"><strong>Sai assistant</strong><button onClick={() => setOpen(false)}>✕</button></div><div className="chat-messages">{messages.slice(-10).map((m) => <p key={m.id} className={`chat-${m.sender}`}>{m.text}</p>)}</div><div className="quick-replies"><button onClick={() => respond("pricing")}>Pricing</button><button onClick={() => respond("faq")}>FAQ</button><button onClick={() => respond("lead")}>Book demo</button><button onClick={() => respond("handoff")}>Human handoff</button></div>{mode === "handoff" ? <form className="chat-form" onSubmit={async (e) => { e.preventDefault(); if (!validateEmail(handoff.email) || handoff.issue.trim().length < 8) return; const t = await createTicket({ email: handoff.email, subject: "Chatbot handoff", message: handoff.issue, priority: "normal", source: "chatbot" }); add("bot", `Ticket ${t.id} created.`); setMode("idle"); }}><input placeholder="Email" value={handoff.email} onChange={(e) => setHandoff({ ...handoff, email: e.target.value })} /><textarea placeholder="Issue" value={handoff.issue} onChange={(e) => setHandoff({ ...handoff, issue: e.target.value })} /><button type="submit">Create ticket</button></form> : null}{mode === "lead" ? <form className="chat-form" onSubmit={async (e) => { e.preventDefault(); if (lead.name.trim().length < 2 || !validateEmail(lead.email) || lead.phone.trim().length < 7) return; const eq = await createEnquiry({ name: lead.name, email: lead.email, phone: lead.phone, subject: "Chatbot lead", message: "Lead requested follow-up.", source: "chatbot" }); add("bot", `Lead captured as enquiry ${eq.id}.`); setMode("idle"); }}><input placeholder="Name" value={lead.name} onChange={(e) => setLead({ ...lead, name: e.target.value })} /><input placeholder="Email" value={lead.email} onChange={(e) => setLead({ ...lead, email: e.target.value })} /><input placeholder="Phone" value={lead.phone} onChange={(e) => setLead({ ...lead, phone: e.target.value })} /><button type="submit">Capture lead</button></form> : null}<form className="chat-input" onSubmit={send}><input value={text} onChange={(e) => setText(e.target.value)} placeholder="Ask a question" /><button type="submit">Send</button></form></div> : null}
...
export default function App() {
  ...
  const createEnquiry = async (input: EnquiryInput) => {
    ...
    setEnquiries((x) => [eq, ...x]); pushToast("success", "Enquiry stored and email notifications queued."); return eq;
  };
